...
function wpcf7_mce_validate_api_key( $input, $logfileEnabled, $idform = '' ) {
  ...
  try {
    ...
    $vc_headers = array( "Content-Type" => "application/json","Authorization" =>"Bearer $dc[0]") ;
    ...
    $resp = wp_remote_get( $urltoken, $opts );
    ...
    if ( isset ( $validate_api_key_response["health_status"] ) ) {
        if ( $validate_api_key_response["health_status"] >=400  ) {
            $tmp = array( 'api-validation' => 0 );
